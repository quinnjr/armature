...
impl Tenant {
    ...
    ///
    /// **Invariant**: `id` must not contain `':'`. [`crate::cache::TenantCache::clear_tenant`]
    /// (and any other consumer that treats `cache_key` output as a prefix) matches
    /// keys with a plain `starts_with("tenant:{id}:")` test, so an `id` containing
    /// `':'` could cause over-matching across tenants. Tenant IDs generated by
    /// [`crate::management::TenantManager::create`] are UUIDv4 strings and
    /// structurally satisfy this; if you construct a `Tenant` directly with a
    /// caller-supplied `id`, make sure it upholds the same invariant.
    ///
    /// # Examples
    ///
    /// ```
