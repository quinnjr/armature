...
- **Version bumps:** `armature-redis`, `armature-diesel`, `armature-seaorm`, `armature-opensearch`, `armature-tenancy`, `armature-eventsourcing`, `armature-queue`, `armature-cron`, `armature-events`, `armature-discovery`, `armature-messaging`, `armature-webhooks`, `armature-push`, `armature-distributed`, `armature-acme`, `armature-payments`, `armature-mail`, `armature-storage`, and `armature-files` are bumped to `0.2.0` for the breaking/behavior/additive-API changes listed below (under Cargo's 0.x semver rules a minor bump signals a breaking change); internal and root dependents pin the new versions.
