...
- **`0.2.0` (breaking):** `armature-admin`, `armature-analytics`, `armature-opentelemetry`, `armature-toon`, `armature-cache`, `armature-collab`, `armature-features`, `armature-i18n`, `armature-metrics`.
- **Patch (additive / behavior-only):** `armature-validation` → `0.1.3`, `armature-ratelimit` → `0.1.4`, `armature-compression` → `0.1.3`, `armature-audit` → `0.1.2`, `armature-config` → `0.1.2`.
- Root-workspace pins on `armature-cache` and `armature-opentelemetry` are moved to `0.2.0` (a `0.1.x` pin no longer resolves); the patch-bumped crates' pins are refreshed to match.
