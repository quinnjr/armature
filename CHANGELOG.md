...
- **`armature-aws` → `0.2.0`:** removed the placeholder `pub mod s3`. Credential-source selection is now behaviorally distinct per variant.
...
- **`armature-lambda` → `0.1.3`** and **`armature-azure-functions` → `0.1.3`** (additive + behavior-only: new accessors, populated fields, 413/504 enforcement; no public removals). Internal dependents (`armature-lambda`→`-aws`, `armature-azure-functions`→`-azure`, `armature-storage`→`-azure`/`-gcp`) repin to the new versions.

##### Known follow-ups (not blocking)

These six crates's vendor SDKs (`aws-smithy-*`/`aws-types` require rustc 1.94.1; the Azure/GCP SDKs pull heavy transitive trees) exceed the previous workspace MSRV of 1.89 — so **the workspace MSRV is raised to 1.94.1** (see the top-level Changed entry) and the crates build under it. They still sit outside the default CI *matrix* (a separate coverage gap tracked in `TODO.md` — the `MSRV Check` job builds only `--features full-with-saml`, not `--workspace`), and live-service paths (real AWS/Azure/GCP round-trips) remain unexercised; the 73 new tests are credential-free (provider/endpoint/parsed-struct construction and synthetic-event conversion), so coverage does not depend on Docker or cloud credentials.
