...
- **Version bumps:** `armature-redis`, `armature-diesel`, `armature-seaorm`, `armature-opensearch`, `armature-tenancy`, and `armature-eventsourcing` are bumped to `0.2.0` for the breaking changes listed below (under Cargo's 0.x semver rules a minor bump signals a breaking change); internal dependents pin the new versions.
