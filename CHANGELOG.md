...
These six crates remain outside the default CI matrix and the MSRV-1.89 gate — the vendor SDKs (`aws-smithy-*`/`aws-types` require rustc 1.94.1; the Azure/GCP SDKs pull heavy transitive trees) exceed the workspace MSRV, so the per-crate gate is `cargo check/clippy/test -p <crate> --all-features` on stable (all green). This pre-existing CI/MSRV-coverage gap (tracked in `TODO.md`) is not closed by this workflow. Live-service paths (real AWS/Azure/GCP round-trips) remain unexercised; the 73 new tests are credential-free (provider/endpoint/parsed-struct construction and synthetic-event conversion), so coverage does not depend on Docker or cloud credentials.
