...
- `armature-storage`: `public_access` applies a predefined ACL, S3 honors `region` and `storage_class`, GCS honors a custom `endpoint` (emulators), `MultipartConstraints` are enforced and re-exported, and new `temporary_url_default` methods honor the configured signed-URL durations.
...
- **Breaking (`armature-storage`):** `AzureBlobStorage::temporary_url` now returns `Ok(None)` instead of a permanent unsigned public URL (the trait contracts a *temporary signed* URL; the old value granted no access to a private container and never expired). `MultipartConstraints` is now exported and actually enforced, so previously-accepted oversized or disallowed multipart uploads are now rejected.
