...
- `armature-http-client`: the interceptor/middleware system is now wired into `execute()`/`send()`; `RateLimitInterceptor` actually waits out a parsed `Retry-After` instead of only logging.
...
**Breaking (Workflow 7, all crates above):** `armature-http-client`'s `HttpClient::execute` now takes a `RequestSpec` rather than a `reqwest::Request`; `Response::from_reqwest` and `RequestBuilder::json`/`form` now return `Result`; `RateLimitInterceptor` is constructed via `new()`/`with_max_wait()` rather than as a unit struct. `armature-grpc`'s `GrpcServerConfigBuilder::build()`/`GrpcServer::bind()` are now fallible. `armature-graphql`'s `create_merged_schema` is removed. `armature-graphql-client`'s `GraphQLClientConfig` loses its `batching`/`max_batch_size`/`batch_delay` fields and builders.
