...
- **Fixed (`armature-security`) — `0.1.2` → `0.1.3`:** `RequestVerifier::verify_request` was verifying signatures against an always-empty body on the zero-copy body-handling path (it read `String::from_utf8_lossy(&request.body)`, but zero-copy requests populate `body_bytes`/the zero-copy body storage instead of `body`). Signature verification was therefore silently broken for every real production request that used the zero-copy path — legitimately signed requests could fail verification. `verify_request` now resolves the body via `request.body_ref()`, which correctly follows zero-copy bodies; covered by a new regression test.
- **Breaking (`armature-jwt`) — `0.1.1` → `0.2.0`:**
  - `refresh_token()` now rejects access tokens presented as refresh tokens: every issued token carries a new `token_use` claim (`"access"` or `"refresh"`), and `refresh_token()` requires `token_use == "refresh"`. Previously a still-valid access token could be laundered into a fresh access/refresh pair via `refresh_token()`; that refresh-token-confusion path is now closed.
  - `sign_standard()` now strips the reserved claim keys (`sub`, `iss`, `aud`, `exp`, `iat`, `jti`) out of caller-supplied `additional_claims` before merging them into the token, preventing claim injection/override of framework-controlled claims. Any caller that was relying on `additional_claims` to override one of these keys will observe different (correct) output.
  - Every issued token's JSON shape changes: all tokens now carry the new `token_use` claim.
  - These are genuinely breaking behavior changes for consumers relying on the old claim-merge or refresh semantics, and are motivated by closing the refresh-token-confusion and claim-injection issues above — hence the `0.2.0` bump rather than a patch.
- **Breaking (`armature-session`) — `0.1.0` → `0.2.0`:** `couchdb_session.rs`'s `doc_id`/`doc_url` now require the session ID to parse as a valid UUID (`uuid::Uuid::parse_str`) before it is used to construct a CouchDB document URL. Previously a session ID containing `/`, `?`, or a CouchDB-reserved prefix (`_design/`, `_local/`, ...) reached the URL unsanitized, a path-traversal/document-ID-injection vector. **Breaking:** callers of `CouchDbSessionStore` using non-UUID session IDs now get `Err(SessionError::InvalidSessionId)` from `get`/`save`/`delete`, where they previously succeeded.
- **Fixed (`armature-mcp`) — `0.1.3` → `0.1.4`:** `authenticate_api_token` now compares tokens in constant time (`constant_time_eq`) instead of via `HashSet::contains`, closing a timing side-channel on API token verification. Non-breaking: pass/fail results are unchanged for correct callers, only timing characteristics change.
- **Fixed (`armature-auth`) — `0.1.2` → `0.1.3`:** `TotpSecret::verify` and `BackupCodes::verify_and_consume` now use constant-time comparison, and the backup-code scan no longer short-circuits on the first match — closing both the direct timing channel on TOTP/backup-code comparison and the subtler channel that previously revealed *which* stored backup code matched via early return. Non-breaking for correct callers.
- **Breaking (`armature-core`) — `0.4.0` → `0.5.0`:** added a new `CompressionAlgorithm::Zstd` variant for Zstandard compression support; `CompressionAlgorithm` is now `#[non_exhaustive]` (see `static_assets.rs`). Adding the variant without `#[non_exhaustive]` would have been a compile-time breaking change for any downstream crate that exhaustively matches on `CompressionAlgorithm` — the more visible `0.5.0` bump signals that source-breaking risk to consumers matching on this enum.

### Changed

- `armature-cli` bumped to `0.4.0`: `generate model` is now a real implementation that writes `src/models/<name>.rs`, updates `mod.rs`, and optionally creates a `migrations/` directory (previously a no-op stub that printed "Model generation is coming soon!" and did nothing). The `cargo add` invocation and the printed "Changes to Cargo.toml" preview text changed to reflect the `armature-framework` package rename (`cargo add armature-framework --rename armature --features <flag>`, previously `cargo add armature --features <flag>`).

### Added

#### Core & Tooling conformance — WF9 (`armature-core`, `armature-proc-macro`, `armature-macros`, `armature-macros-utils`, `armature-testing`, `armature-app`, `armature-cli`, `armature-ferron`, `armature-rhai`, `armature-log`)
- `armature-proc-macro`: the documented `#[catch]`, `#[guard]`, `#[use_guard]`, `#[middleware]`, and `#[use_middleware]` decorators are now compiled and exported (their source files existed but were never `mod`-declared, so following armature-core's own docs failed to compile). `#[cache]` parses its documented `ttl`/`key`/`tag` attributes, keys on real argument values, and actually returns cached payloads. `Param` derive supports the documented multi-field struct mode; `Controller::routes()` returns the real route list; trybuild + behavioral suites cover every exported macro.
- `armature-macros`: the headline `routes!` macro is rewritten onto `Route::new` — previously **any** use of `routes!` failed to compile (the crate's only test was an empty function). Every exported macro now has a behavioral test.
- `armature-macros-utils`: every documented macro form now parses and works — multi-arg `bail!`/`ensure!`/`validate!`/`validate_required!`, status-prefixed `json!`/`html!`/`text!`/`redirect!`, real `test_request!`; `assert_json!`/`assert_status!` genuinely assert (they expanded to nothing). `#[api(skip)]` excludes fields under every serde `rename`/`rename_all` style, including split `rename(serialize=..)` forms.
- `armature-core`: additive `Application::listen_on(addr)` (bind any address, not just `0.0.0.0`); `Module::module_type_id`/`module_type_name` default methods; `CacheInterceptor` backed by a real TTL store; `micro::Compress` and `CompressionMiddleware` perform real gzip (Accept-Encoding-negotiated, `Vary`-merged); HTTP/3 flow-control windows and 0-RTT are applied to the quinn config.
- `armature-testing`: `TestApp`/`TestContainer` DI is real (registered providers resolve and reach handlers — they previously returned unconditional `None` against an empty container); `add_module` registers recursively with concrete-`TypeId` dedup; `IntegrationTestBuilder::run_test` executes the stored hooks; the load runner honors `rate_limit` and issues exact request counts; `DockerContainer::start` polls readiness up to the configured timeout.
- `armature-app`: module-level guards are enforced ahead of controller guards; the `host` knob is parsed and bound (127.0.0.1 actually binds 127.0.0.1); module setters and bootstrap hooks fail loudly instead of silently.
- `armature-rhai`: dynamic-route `:param`/catch-all captures reach `request.param(..)` (the Quick Start's headline feature — previously always UNIT); `call_after` exposes the real outgoing response (status/headers/body/cookies); `script_handler` never panics (runtime-flavor-aware blocking); catch-alls match on segment boundaries; hot reload recompiles only stale scripts.
- `armature-cli`: the four advertised-but-aborting templates (graphql/grpc/lambda/cloudrun) are implemented, with template validation before any file is written; wizard selections and `--database`/`--docker`/`--ci` emit real deps, Dockerfile, and CI workflow; `--fields`, `--guard-type`/`--job-type`/`--auth` shape generated code (against the real `Guard`/`armature-cron` APIs); `routes` gains real `--path`/`--format`/`--middleware` handling with parsed (not hardcoded-zero) statistics; generated OpenAPI clients honor `--with-logging`/`--with-retry`/`--base-url`; `mock --watch` reloads on spec changes; the REPL injects the Armature prelude.
- `armature-ferron`: `to_kdl()` emits every documented option (backend timeout/headers on both paths, ports, error_log, logging gate, add_prefix rewrite); auto-reload spawns a real debounced `notify` watcher with a `reload_count()` stat and a clean shutdown path; health transitions honor the configured thresholds; `is_running` uses a signal-0 probe with zombie reaping.
- `armature-log`: `ARMATURE_*` env configuration takes effect automatically on first macro use (as documented — previously silently ignored without an explicit `init()`); color auto-detection performs a real `IsTerminal` check; JSON/Pretty/Compact renderers are test-covered.
...
- **Version bumps (WF9):** `armature-core` → `0.3.0` and `armature-cli` → `0.3.0`; `armature-proc-macro`, `armature-macros`, `armature-macros-utils`, `armature-testing`, `armature-ferron`, `armature-rhai`, and `armature-log` → `0.2.0` (0.x minor = breaking/behavior change per the project's semver convention); internal and root dependents pin the new versions.
- **Breaking (`armature-core`): `dependency-injector` upgraded from `1.0.0` to `2.1.0`.** This is a breaking release upstream (`ProviderRegistration::register_fn`'s signature changed in `2.0.0`, per the `dependency-injector` crate's own changelog). `armature-core` re-exports several of `dependency-injector`'s types verbatim (`Container` as `DiContainer`, `DiError`, `Factory`, `Injectable`, `Lifetime`, `Provider`, `Scope`, `ScopeBuilder`, `ScopedContainer` as `DiScopedContainer`), so the breakage is directly visible to `armature-core` consumers as well — `armature-core`'s own version is bumped `0.3.0` → `0.4.0` accordingly (0.x minor = breaking change per the project's semver convention).
- **Breaking (`armature-core`):** `BatchConfig` loses `adaptive_batching`, `min_batch_size`, and `parse_timeout_ms` (fields, setters, and preset references) — they were read nowhere. Behavior: `micro::Compress`/`CompressionMiddleware` now actually compress; `CacheInterceptor` actually caches; `Application::register_module` dedups by **concrete** module type, so distinct imported modules all register (previously every module after the first was silently dropped — `type_name_of_val` on `&dyn Module` is one shared string).
- **Breaking (`armature-macros-utils`):** `#[derive(Model)]` now emits `Debug` + `Clone` impls (structs also deriving them must drop the duplicates; the Serialize/Deserialize claim is retracted — a derive macro cannot attach other derives). `Resource::table_name()` returns the configured/snake-case table name instead of the struct identifier. `html!`/`text!`/`redirect!` pin the `Ok` error type to `armature_core::Error`. `assert_json!`/`assert_status!` now really assert, so tests that silently passed against wrong responses will now fail (that is the point).
- **Breaking (`armature-proc-macro`):** `#[cache]` keys on argument values and honors `ttl`/`key`/`tag` — call sites relying on the old function-name-only key (all argument values shared one entry, returning wrong data) change behavior; cached payloads are now actually returned. `Controller::routes()` returns the real (controller-relative) route list instead of `vec![]`. `Query` derive round-trips values containing `&`/`=`/`%`.
- **Breaking (`armature-proc-macro`) — security:** `#[guard(...)]` and `#[middleware(...)]` on a controller **struct** now actually enforce. Previously they generated guard/middleware factory metadata that the `#[module]` route registrar never consumed, so a controller annotated `#[guard(AuthGuard)]` compiled and served **every route unguarded** while the docs claimed it was protected. The registrar now runs a controller's struct-level guards ahead of the handler (first guard returning not-activated or erroring → 403) and applies its middleware as a chain. Any controller that was relying — knowingly or not — on those guards being inert will now have them enforced. Guards and middleware are now constructed **once per route at registration**, not once per request (the initial enforcement fix rebuilt them, and cloned the request once per guard, on every request); a guard's own `Err` now propagates to the framework's normal error response instead of being collapsed into the same hardcoded 403 as a plain `Ok(false)` denial — since the built-in `AuthenticationGuard`/`RolesGuard` return `Err(Forbidden(..))` rather than `Ok(false)`, their 403 body changes from the plain-text `Forbidden` to the standard JSON error shape (status stays 403). The dead `__GUARDS_<NAME>`/`__MIDDLEWARES_<NAME>` count constants (never read) are removed.
- **Breaking (`armature-macros`):** `routes!` is rewritten onto `Route::new` — no working caller existed (the old expansion could not compile). Callers of `log_error!` need their own `tracing` dependency (macro paths resolve at the call site; documented).
- **Breaking (`armature-ferron`):** `FerronConfig::backend` is `Option<Backend>` (was `Option<String>`); `FerronManagerBuilder::build()` returns `Result<Arc<FerronManager>>`. Unused `kdl`, `handlebars`, and `futures` dependencies dropped.
- **Breaking (`armature-app`):** `ScriptModule` setters return `Result` and error on wrong-typed elements (previously silently dropped); new `AppError::InvalidHost`; a failing bootstrap hook aborts startup; the `host` knob is honored (a non-default host previously silently bound `0.0.0.0`). Docs now say `ctx.invoke(...)` — registering the documented `ctx.call` is impossible because `call` is Rhai's reserved function-pointer keyword, intercepted before user registrations (pinned by a test).
- **Breaking (`armature-cli`):** the `armature db` subcommand and the README "Database Migrations" claim are removed (every arm printed "coming soon!"); `ClientOptions.async_client` removed (generated clients are async, the flag did nothing); duplicate clap aliases renamed (`validate`'s `check` collided with `doctor`'s, panicking every debug invocation); `new::run` gained a project-options parameter; generated projects and the REPL pin `armature = "0.2"`.
- **Breaking (`armature-rhai`):** `call_after` scripts now see the real outgoing response (previously an empty one) and `ResponseBinding` round-trips cookies; `/api/*` no longer matches `/apix`; hot reload serves cached unchanged scripts instead of recompiling per request; `script_handler` no longer panics outside a multi-thread runtime (dedicated-thread fallback, with a documented per-call cost).
- **Changed (`armature-testing`):** `TestApp::get`/`TestContainer::get` resolve from the real container and `TestApp::new` shares the caller's container with the app; `DockerContainer` auto-stop on drop is a synchronous `docker stop` (no more nested-runtime panic inside `#[tokio::test]`); the load runner distributes remainders exactly and paces with N−1 gaps; module wiring dedups by concrete `TypeId` and caps import depth at 64.
- **Changed (`armature-core`) — `CacheInterceptor` safety:** the opt-in response-cache interceptor now caches only `GET`/`HEAD`, includes the (canonicalized) query string in its key, and refuses to cache responses carrying `Set-Cookie`, `Cache-Control: private`/`no-store`, a non-empty `Vary`, or a non-empty `Content-Encoding` — closing a cross-user body/session-cookie bleed if it was mounted on an authenticated route, and preventing a negotiated/encoded representation (the key does not fold in request headers such as `Accept-Encoding`) from being replayed to a caller that never negotiated it; its doc now warns against per-user routes. The key also length-prefixes the path (`METHOD:{path.len()}:path`) so a path containing a literal `?` can never collide with a distinct, shorter path plus query parameters that happen to join to the same string. Separately, `ResponseCache` eviction no longer evicts a recently-refreshed entry by its original insertion time (a refresh correctly resets eviction recency), and gzip compression (`micro::Compress`, `CompressionMiddleware`) offloads bodies above 32 KiB to `spawn_blocking` so a large response can't stall the async runtime (output bytes unchanged); the offload path shares the body via `Arc` instead of cloning it into the blocking task, and correctly reads/restores whichever of `HttpResponse`'s two body storages (`body` or the zero-copy `body_bytes`) is actually populated — the initial offload fix read only the legacy `body` field, so a response built via `with_bytes_body`/`with_static_body` (static-file serving, `ResponseBuilder`, `FastResponse`) would be gzip-compressed as if empty, silently discarding its real content.
- **Fixed (`armature-ferron`):** the config-file watcher no longer forms a self-perpetuating reload loop when `auto_reload` is combined with a service registry (it previously regenerated and rewrote the watched file on every file-change event, re-triggering itself); the watcher now signals Ferron to re-read the file, and an external edit is honored instead of being clobbered by registry regeneration.
- **Changed (`armature-log`):** `LogConfig::from_env()` is now a pure function — it no longer syncs two global atomics as an undocumented side effect (env config is applied by `init()`/`config()`/first macro use instead).
- **Breaking (workspace MSRV): minimum supported Rust raised from `1.89` to `1.94.1`.** The Cloud Platforms crates' vendor SDKs (`aws-smithy-runtime-api`/`aws-smithy-types`/`aws-smithy-schema`/`aws-types`) require rustc `1.94.1`, so the workspace could not honestly claim `1.89` while those members were part of it. `[workspace.package].rust-version` and the umbrella `[package].rust-version` are now `1.94.1`, the CI `MSRV Check` job runs on the `1.94.1` toolchain, and `armature-mcp`'s bespoke `1.91` override (needed only to clear the old `1.89` floor) is dropped in favour of inheriting the workspace value. Verified: the umbrella `--features full-with-saml` and every cloud crate `--all-features` build on `1.94.1`.
...
#### Reconciliation follow-up — missed version bumps (`armature-admin`, `armature-cache`, `armature-queue`, `armature-tenancy`, `armature-distributed`, `armature-messaging`, `armature-toon`)

A code-review pass over this reconciliation diff found that several of the fixes above changed public API or observable behavior in ways that should have bumped the affected crate's own version, but did not — the same class of gap already caught and fixed for `armature-core` (`0.3.0` → `0.4.0`, see the `dependency-injector` entry above). The same treatment is applied here:

- **Version bumps (reconciliation):** `armature-admin`, `armature-cache`, `armature-queue`, `armature-tenancy`, `armature-distributed`, `armature-messaging`, and `armature-toon` are bumped `0.2.0` → `0.3.0` for the breaking/behavior changes described below (0.x minor = breaking change per the project's semver convention); the root workspace's version-pinned optional dependencies (`armature-cache`, `armature-queue`, `armature-messaging`) are refreshed to match.
- **Breaking (`armature-admin`):** `ActivityItem`, `ActivityAction` (and their impls), `DashboardView::recent_activity`, and `DashboardView::with_activity()` are removed. This is dead scaffold code that was never wired into the real dashboard, correctly removed per the original audit — but all four were `pub` and reachable via `pub use dashboard::*`, so any consumer that referenced them will fail to compile.
- **Breaking (`armature-cache`):** `TaggedCache::get_keys_by_tag`, `get_tags_for_key`, and `list_tags` now return `CacheResult<Vec<String>>` (were `Vec<String>`), so a backend error on a tag-index lookup surfaces instead of being indistinguishable from "no keys" — callers must add `?` or otherwise handle the `Result`. (`TieredCache`/`RedisCache::clear()` scoping is also additionally hardened in this release.)
- **Breaking (`armature-queue`):** `Worker::stop()`'s runtime behavior changed from near-instant (`handle.abort()` on every in-flight task, returning immediately) to blocking for up to `job_timeout` (default 300s) while in-flight jobs drain gracefully, before force-aborting stragglers. The signature is unchanged, but callers operating under a fixed external shutdown budget (e.g. a container orchestrator's grace period) can now be killed mid-drain if that budget is shorter than `job_timeout` — worth flagging since it's easy to miss just reading the doc comment. `stop_with_timeout`'s return type may also have changed in this release — see the `worker.rs` changes.
- **Breaking, security (`armature-tenancy`):** `Tenant::cache_key`'s output format changed from `"tenant:{id}:{key}"` to a length-prefixed `"tenant:{id.len()}:{id}:{key}"`, closing a cross-tenant cache-key-collision bug where a crafted tenant id/key pair could resolve to another tenant's cache entry. Same signature, different output: cache entries written under the old format become unreachable (self-healing via TTL, low risk) but, more importantly, `TenantCache::clear_tenant` — built from the new format — can no longer match old-format keys. **Operators upgrading past this version should consider flushing/rebuilding the tenant cache** to ensure old-format keys (which `clear_tenant` can no longer target) don't linger longer than their TTL.
- **Breaking (`armature-distributed`):** `LeaderElection`'s `on_elected`/`on_revoked` callback dispatch changed from blocking/inline-awaited to detached (`tokio::spawn`, fire-and-forget); existing callback implementations relying on serialization or non-reentrancy against the previous inline-await behavior may now observe callbacks running concurrently with subsequent election state changes. Separately, `LeaderFence` (returned by `LeaderElection::fence()`) is now properly re-exported at the crate root alongside `LeaderElection`/`LeaderElectionBuilder`/`LeaderError` — it was previously reachable only via the `leader` module path despite being public API.
- **`armature-messaging`:** two previously-documented no-op config fields now work as originally documented: `SubscribeOptions::concurrency` (previously silently ignored with a warning; now genuinely dispatches handlers concurrently across all five broker backends) and `NatsConfig::jetstream` (previously had no effect on `publish`; now routes through JetStream when set). Both remain opt-in — the default `None`/`false` preserves the old behavior exactly — but an existing caller who had already set either field expecting it to work will now see genuinely different (and correct) behavior.
- **`armature-toon`:** `accepts_toon`/`prefers_toon` content negotiation now does proper RFC 7231 `q=`-value-aware `Accept` header parsing instead of naive substring/positional matching — e.g. `accepts_toon(Some("application/toon;q=0"))` now correctly returns `false` (it previously, incorrectly, returned `true`). Signatures are unchanged; this is a correctness fix that can change which content type gets served for `Accept` headers that were previously parsed incorrectly.
...
- `armature-rhai`: `ResponseBinding` stores its body as `bytes::Bytes` instead of `Vec<u8>`, so a `call_after` scope clone (seeding both `response` and `original_response`) is an O(1) refcount bump rather than a full body copy, and `from_http_response` is zero-copy when the source response is already `Bytes`-backed (private field; not a public API change).
...
#### Cloud Platforms conformance (Workflow 5) — `armature-aws`, `armature-azure`, `armature-gcp`, `armature-lambda`, `armature-cloudrun`, `armature-azure-functions`

A conformance pass across the six cloud-platform crates closed all 43 findings (6 Critical, 19 Warning, 18 Info): advertised-but-unwired services, credential sources accepted then ignored, config knobs read nowhere, runtime data never populated, non-compiling READMEs, and three crates (`-aws`, `-cloudrun`, `-gcp`, `-lambda`, `-azure-functions`) that had no tests at all. Each `reconcile: code` fix carries a regression test that fails against the old code (73 new tests total); the implement-vs-remove call for each service/credential finding was decided against the crate's **pinned** SDK version, not the latest upstream docs.

**Unwired services made real (or honestly removed).** `armature-azure` advertised Service Bus (`enable_servicebus()` + a re-export) with no field/init/accessor; it now has a real `ServiceBusClient` (implemented against `azure_messaging_servicebus 0.21`'s SAS model — the 0.21 line has no token-credential path — vending `queue()`/`topic()` clients from a namespace + SAS policy/key or connection string) wired through a `servicebus()` accessor. `armature-gcp`'s four hollow `enable_*` builders are resolved by the pinned SDK: Secret Manager, Cloud Run, and Cloud Functions are now real authenticated REST clients (`SecretManagerClient`/`CloudRunClient`/`CloudFunctionsClient`), while **Firestore was removed** — `google-cloud-googleapis 0.16` generates no Firestore client, so the `firestore` feature, `enable_firestore()`, and the now-unused `google-cloud-googleapis`/`tonic` deps were dropped rather than left as a dead knob.

**Credential sources honored.** `armature-aws` now maps `Environment`/`IamRole`/`WebIdentity` to distinct real providers (`EnvironmentVariableCredentialsProvider`, IMDS, web-identity-token — verified present in the locked `aws-config 1.8`), where the old code collapsed all four non-`Auto` variants into the default chain. `armature-gcp` threads `config.credentials` (service-account JSON/file, metadata server, and — for the REST clients — static access token) into every client builder instead of hardcoding ADC. `armature-azure` **removed** `ConnectionString` and `StorageAccountKey` credential variants: `azure_storage_* 1.0` exposes no connection-string/shared-key constructor and `azure_identity 1.0` dropped `EnvironmentCredential`, so those advertised paths were dead and are deleted, with `Environment` re-documented to reflect the developer-tools chain it actually resolves through.

**Config knobs wired; runtime data populated.** `armature-gcp` applies `emulator_host` and per-service `service_configs[..].endpoint` as endpoint overrides. `armature-azure-functions` enforces `max_request_size` (413 on oversize, checked against both `Content-Length` and the collected body) and `timeout_seconds` (504 via `tokio::time::timeout`), and raises request logging from `debug!` to `info!` so the default filter actually emits it. `armature-lambda` now populates Cognito/JWT authorizer `claims()` (V2 `jwt.claims`, V1 authorizer map), `path_parameters`, and `stage_variables` (all previously hardcoded empty), and `impl_request_handler!` forwards the full request (headers/query/context), not just method/path/body. `armature-cloudrun` gained a real health-check HTTP endpoint (`HealthCheck::serve`/`handle_request` over hyper — the "built-in endpoint" claim was previously hollow), reads `service_url` from `CLOUD_RUN_URL` instead of fabricating a wrong-but-plausible URL, makes `InstanceMetadata::fetch` surface an error when the metadata server is unreachable (was always `Ok(all-None)`), and fetches its metadata fields concurrently.

**Non-compiling READMEs / doc examples rewritten** across all six crates to the real `*Config::builder()`/`*Services::new` (or `LambdaRuntime`/`CloudRunConfig`) surface shown correctly in each `lib.rs`; the fabricated `S3Client::new`/`BlobClient::new`/`StorageClient::new`/`CloudRunApp`/`LambdaRuntime::api_gateway` types are gone. `armature-azure-functions`' inert `bindings` concrete types (never re-exported, wired into nothing) were deleted and the "access Azure services through bindings" / Timer-Queue-Blob-trigger claims softened to the HTTP-only reality; `armature-aws`' placeholder `pub mod s3` was removed (real access is `AwsServices::s3()`).

##### Breaking (Workflow 5)

Under Cargo's 0.x rules a minor bump signals a breaking change.

- **`armature-azure` → `0.2.0`:** removed the `CredentialsSource::ConnectionString`/`StorageAccountKey` variants and the `connection_string()`/`account_key()` builders (the SDK supports neither). New public surface: `ServiceBusClient`, `ServiceBusServiceConfig`, `AzureServices::servicebus()`/`cosmos_database()`, `AzureConfig::servicebus_connection_string`. New dep `azure_core` 0.21 (aliased `azure_core_sb`, rustls-only) for the Service Bus transport.
- **`armature-gcp` → `0.2.0`:** removed the `firestore` feature + `enable_firestore()` (+ `google-cloud-googleapis`/`tonic`/`jsonwebtoken` deps). New public REST client types + accessors for secret-manager/cloud-run/cloud-functions; `AccessToken` credential is rejected (error) for the gRPC storage/pubsub clients (honored for REST). New dep `google-cloud-auth 1.14`.
- **`armature-aws` → `0.2.0`:** removed the placeholder `pub mod s3` plus its eleven sibling stub modules (`dynamodb`/`sqs`/`sns`/`ses`/`lambda`/`secrets`/`ssm`/`cloudwatch`/`kinesis`/`kms`/`cognito` — each a one-line placeholder; real access is `AwsServices::<svc>()`). Credential-source selection is now behaviorally distinct per variant, and `AwsConfig::from_env()` follows the AWS-standard precedence (environment static credentials → web-identity → profile, treating an empty `AWS_PROFILE` as unset).
- **`armature-cloudrun` → `0.2.0`:** `CloudRunConfig` gained a public `url` field; `InstanceMetadata::fetch` now returns `Err` when the metadata server is unreachable. New public health-endpoint API.
- **`armature-lambda` → `0.2.0`:** the exported `impl_request_handler!` macro now expands to `self.handle_request(request)` rather than `self.handle_request(method, &path, body)`, changing the `handle_request` signature every consumer must implement — a breaking macro-contract change. New behavior: populated Cognito/JWT `claims()`, `path_parameters`, and `stage_variables`.
- **`armature-azure-functions` → `0.2.0`:** removed the re-exported `InputBinding`/`OutputBinding` traits and the `bindings` module (dead scaffolding — no implementors, never dispatched by the runtime). Behavior: `max_request_size` (413) and `timeout_seconds` enforcement, with the size cap now applied during streamed/chunked ingestion (via a bounded `Limited` body) instead of after full buffering, and the timeout now also covering body ingestion (408 on a stalled upload) alongside the handler (504).
- Internal dependents (`armature-lambda`→`-aws`, `armature-azure-functions`→`-azure`, `armature-storage`→`-azure`/`-gcp`) repin to the new versions; no in-workspace crate pins a version on `armature-lambda` or `armature-azure-functions`, so their bumps need no further repin.

##### Post-review hardening (Workflow 5)

A multi-dimensional review of the pass above surfaced fixes now folded in:

- **Resilience / timeouts.** The `armature-gcp` REST clients (Secret Manager / Cloud Run / Cloud Functions) build with an explicit request + connect timeout — previously untimed, so a stalled endpoint hung the calling task indefinitely — and their HTTP-status handling now maps `401|403 → Auth`, `404 → NotFound`, `429 → RateLimited`, `5xx → ServerError` (new `GcpError` variants) instead of collapsing every status into one error. `armature-azure`'s `ServiceBusClient` injects a 30 s-timeout reqwest client (rustls-only). `armature-cloudrun`'s health server no longer dies on a transient `accept()` error (logs and continues), and `InstanceMetadata::fetch` surfaces a metadata-server `5xx`/`429` as an error rather than a silently-incomplete result.
- **Request-size / timeout hardening (`armature-azure-functions`).** The `max_request_size` cap is enforced *during* ingestion via a bounded body, so a chunked upload with no `Content-Length` can no longer OOM the process before the check fires; the request timeout now also bounds body ingestion (408 on a stalled upload); and a mid-stream abort returns 400 instead of being processed as a silently-empty request.
- **Security hygiene.** `armature-gcp` percent-encodes caller-supplied REST path segments and defaults bare-host endpoint overrides to `https://`; `armature-azure`'s connection-string parse error redacts the offending segment so key material cannot reach a log.
- **Correctness / consistency.** `armature-aws` `AwsConfig::from_env()` credential precedence now matches the AWS SDK chain, and its `initialize_enabled_services` propagates errors instead of discarding them; internal duplication was consolidated (`armature-gcp` shared `RestClient` + `rest_service!` macro, `armature-aws` `get_or_init!` accessor macro) with no public-API change. New credential-free regression tests pin the 413 boundary (exact-limit and one-over, both paths), the GCP REST request path + `Authorization` header, and the healthy readiness path; **all six crates now run in the CI `test-members` matrix under `--all-features`.**

##### Known follow-ups (not blocking)

These six crates' vendor SDKs (`aws-smithy-*`/`aws-types` require rustc 1.94.1; the Azure/GCP SDKs pull heavy transitive trees) exceed the previous workspace MSRV of 1.89 — so **the workspace MSRV is raised to 1.94.1** (see the top-level Changed entry) and the crates build under it. All six are now in the CI `test-members` matrix and run under `--all-features`, so the feature-gated conformance suites — previously compiled nowhere, since the `coverage` job builds members with default features only — actually execute. Live-service paths (real AWS/Azure/GCP round-trips) remain unexercised by design; the new tests are credential-free (provider/endpoint/parsed-struct construction and synthetic-event conversion) and use loopback stubs, so coverage does not depend on Docker or cloud credentials.

#### Platform Services (Workflow 8) — `armature-cache`, `armature-ratelimit`, `armature-compression`, `armature-metrics`, `armature-opentelemetry`, `armature-audit`, `armature-analytics`, `armature-config`, `armature-features`, `armature-i18n`, `armature-validation`, `armature-toon`, `armature-collab`, `armature-admin`

A conformance pass across the 14 platform-services crates closed 103 findings: request-path panics, headline capabilities that were hollow, telemetry/data that diverged from reality, inert configuration knobs, and READMEs whose examples did not compile. Each fix is backed by a regression test that failed against the old code.

**Request-path panics fixed.** Three crates could be crashed by attacker-controlled multibyte input on the hot path: `armature-metrics`' `RequestMetricsMiddleware` sliced the request path at a fixed byte offset (`&path[..97]`), `armature-audit`'s `mask_value` sliced secrets by byte, and `armature-ratelimit`'s fixed-window algorithm divided by `window.as_secs()` and panicked on any sub-second window. All three now truncate on UTF-8 char boundaries / compute in milliseconds. `armature-ratelimit`'s IP rate limiting also silently failed open on any proxy chain — the whole `X-Forwarded-For` header was parsed as one `IpAddr` and, on failure, the key became `None`; it now parses the first hop — and `skip_on_error(false)` now genuinely fails closed.

**Hollow headline capabilities made real.** `armature-admin`'s `routes()` returned a wrapper that mounted nothing with hardcoded-empty list rows; it now builds a real `armature_core::Router` (dashboard/list/detail/create/edit/delete) backed by a pluggable `DataSource`, with working pagination, `require_auth` gating, and CRUD that mutates the store. `armature-analytics`' `AnalyticsMiddleware` implemented no `Middleware` trait (nothing was ever recorded automatically); it now records every request (errors as 500), normalizes high-cardinality paths, and honors its capacity/toggle config. `armature-features` gained a real LaunchDarkly provider behind the `launchdarkly` feature (rustls-only) and a real regex `Operator::Matches` (was hardwired `false`), plus deterministic multivariate bucketing. `armature-validation` gained the documented `#[derive(Validate)]` macro (in `armature-proc-macro`, re-exported) supporting `length`/`range`/`email`/`url`/`regex`/`required`/`custom`/nested. `armature-toon`'s `to_string_pretty` and serializer knobs now produce genuinely distinct output. `armature-metrics`' `Summary` was a placeholder unit struct and is now a real φ-quantile summary metric, and process metrics register into the exported registry.

**Divergent data corrected.** `armature-opentelemetry`'s middleware recorded the literal strings `"method"`/`"path"` as span dimensions (only on success); it now records the real method/route and counts error requests, and `init_tracing` installs a `TraceContextPropagator` so inbound `traceparent` is actually parsed (was a Noop). `armature-analytics`' latency truncated sub-millisecond requests to zero, its endpoint/client caps froze *all* counters once full, `avg_utilization` reported an allow-ratio, `requests_last_hour` was `last_minute × 60`, and sampling used a biased `subsec_nanos() % 1000` — all corrected. `armature-collab`'s RGA merge is now deterministic (node-set union + a pre-order rebuild, so replicas converge regardless of merge direction), and deletions keep the char as a tombstone so a delete replays as a real `Delete` op instead of resurrecting as `Insert '\0'` — a data-corruption fix. `armature-cache`'s `TieredCache::stats()` now exposes live hit/miss/promotion counters instead of echoing config booleans.

**Inert configuration knobs made real, unbounded growth bounded.** `armature-cache` applies Redis `connection_timeout`/`operation_timeout`/`max_connections` and bounds `InMemoryCache` (was unbounded, expired keys never evicted); `armature-ratelimit` bounds its in-memory algorithm maps (a network-reachable OOM) and honors `key_prefix`; `armature-compression` honors streaming `min_chunk_size` and `Auto::compress` errors instead of silently emitting uncompressed bytes; `armature-config` resolves dotted keys into nested config, coerces string-stored env values for typed getters, and no longer swallows a failed `.env` load; `armature-audit`'s file backend reuses one buffered handle (was `open(2)` per event), implements real `delete_before` so retention works, and decodes the real JWT subject instead of a constant; `armature-i18n` fixes number formatting, emits weekdays, uses locale-aware month/weekday names, shows timezones, and negotiates the best-scoring locale; `armature-validation` length rules count Unicode characters, not bytes.

**Non-compiling READMEs rewritten** across `armature-cache`, `armature-compression`, `armature-metrics`, `armature-opentelemetry`, `armature-audit`, `armature-config`, `armature-features`, `armature-i18n`, and `armature-validation` — examples referenced types/constructors that never existed and now compile against the real API. **Retractions:** genuinely-absent claims were removed rather than stubbed — `armature-opentelemetry`'s logging integration (feature + dep dropped), `armature-audit`'s query-builder/DB-backend docs, `armature-config`'s hot-reload/secrets/remote-source/YAML claims, `armature-compression`'s async-buffering, and `armature-validation`'s "2-4× speedup" claim.

##### Breaking (Workflow 8)

Under Cargo's 0.x rules a minor bump signals a breaking change. Public-surface changes:

- **`armature-admin`:** `AdminRoutes` removed; `AdminInstance::routes()` now returns `armature_core::Router`; `ListView::new` gained a `per_page` parameter; `FieldType::sql_type` removed; `AdminInstance` gained a required `data_source` field (construct via `Admin::build()`).
- **`armature-analytics`:** `TrackedHandler` and the `AnalyticsExt` trait removed — tracking is now via the `Middleware` impl on `AnalyticsMiddleware`.
- **`armature-opentelemetry`:** the `logging` Cargo feature and `TelemetryConfig::enable_logging` field are removed (`full` no longer pulls `logging`; the `opentelemetry-appender-tracing` dep is dropped).
- **`armature-toon`:** `ToonDeserializer::deserialize`/`deserialize_bytes` now require `T: DeserializeOwned + Serialize` (to enforce `strict` unknown-field rejection); parse/type errors now classify as `DeserializeError` (were mislabeled `SerializeError`).
- **`armature-cache`:** `CacheStats` gained `l1_hits`/`l2_hits`/`misses`/`promotions` public fields.
- **`armature-collab`:** `CharNode` gained a `deleted` field and `SyncMessage::Operation` gained a `deps` field (both `#[serde(default)]`, so the wire format stays backward-readable; exhaustive Rust construction/patterns must update).
- **`armature-features`:** `FeatureFlag` gained an `experiment` field (`#[serde(default)]`).
- **`armature-i18n`:** `DateFormatter` gained a `timezone` field; new public `TimeZone` type.
- **`armature-metrics`:** `Summary` is no longer a placeholder — `Summary::new` now takes `(name, help)` and returns `Result`, and its `Default` impl is removed; use `register_summary`/`register_summary_with_opts`.
- **`armature-ratelimit`:** `RateLimitError` gained a `Timeout` variant and `RateLimitConfig` gained `operation_timeout` and `trusted_proxy_depth` fields; both types are now `#[non_exhaustive]` (future variants/fields won't break downstream `match`es or struct literals — construct `RateLimitConfig` via `Default::default()` plus field updates). The public `RateLimitMiddlewareBuilder` was removed (adversarially-verified dead code). The `trusted_proxy_depth` default (`0` = ignore `X-Forwarded-For`) is also a default-behavior change — see the follow-up section.

##### Version bumps (Workflow 8)

- **`0.2.0` (breaking):** `armature-admin`, `armature-analytics`, `armature-opentelemetry`, `armature-toon`, `armature-cache`, `armature-collab`, `armature-features`, `armature-i18n`, `armature-metrics`, and `armature-ratelimit` (see the follow-up section below — a new `RateLimitError::Timeout` variant, two new `RateLimitConfig` fields, `#[non_exhaustive]` on both, removal of the public `RateLimitMiddlewareBuilder`, and a security-hardened X-Forwarded-For default together push it from a patch to a minor).
- **Patch (additive / behavior-only):** `armature-validation` → `0.1.3`, `armature-compression` → `0.1.3`, `armature-audit` → `0.1.2`, `armature-config` → `0.1.2`.
- Root-workspace pins on `armature-cache`, `armature-opentelemetry`, and `armature-ratelimit` are moved to `0.2.0` (a `0.1.x` pin no longer resolves); the patch-bumped crates' pins are refreshed to match.

#### Platform Services follow-up (Workflow 8, four-skill audit battery)

A `/simplify` + `/optimize` + conformance `/audit` + full 7-domain `/code-review` battery over the WF8 branch confirmed the conformance work is genuinely real (the "made-real" claims are implemented and test-backed, independently verified), and surfaced a further set of security, correctness, and performance issues introduced by or adjacent to the WF8 fixes — all now closed with regression tests.

**Security / correctness.** `armature-ratelimit`'s **Redis sliding-window limiter could be bypassed under load**: the Lua script used the millisecond timestamp as both the sorted-set score and member, so requests in the same millisecond collapsed to one entry (`ZCARD` under-counted) and admitted far beyond the limit — fixed with a unique member (`<now>-<seq>` via an `INCR`), verified by a 200-concurrent-same-millisecond burst test. `armature-audit`'s `extract_user_id`, made to "reflect the real principal" in WF8, derived that principal from an **unverified** JWT payload — spoofable, defeating audit non-repudiation; it now prefers a signature-verified `UserContext` from the request extensions and only falls back to unverified decoding behind an opt-in `trust_unverified_jwt_subject` flag (default off). `armature-ratelimit` **unconditionally trusted `X-Forwarded-For`** (a client-controlled header) for IP keying — trivially bypassed by rotating the first hop; a new `trusted_proxy_depth` config (default **0 = ignore forwarding headers**) selects the correct hop from the right, so XFF is now opt-in — a deliberate **default-behavior change** (callers behind a proxy must set the depth). `armature-collab`'s op-based `apply()` dropped a `Delete` that arrived before its `Insert` (resurrecting deleted text); it now records a tombstone placeholder so out-of-order delivery converges. `armature-audit`'s secret masking missed dashless SSNs and Amex/Diners card lengths and leaked the last four SSN digits — all closed.

**Performance.** `armature-analytics`' per-request `record_request` scanned the timestamp deque under the write lock (≈O(n²) + thread serialization on the hot path) and that deque was count-unbounded — both fixed by switching to per-second buckets. `armature-ratelimit` gained a **per-operation Redis timeout** (mirroring the cache crate) so a partitioned Redis surfaces a `RateLimitError::Timeout` and the `skip_on_error` guard engages instead of the request stalling forever; its Redis `fixed_window` was collapsed from 4 round-trips (plus a TTL-leak race) into one atomic script, `reset` batches its fixed keys into a single variadic `UNLINK` (windowed keys still need a `SCAN` sweep), and `remaining` folded to ≤3 round-trips. `armature-features`' `Operator::Matches` no longer recompiles its regex on every evaluation (thread-local cache); `armature-cache`'s Memcached backend gained a native batch `mget` (was N serial round-trips) and uses native `touch` for `expire` (was read-then-write); `armature-metrics`' `sanitize_path` uses an `RwLock` (was a global `Mutex` per request); assorted quickselect/allocation trims across analytics, audit, and metrics.

**Conformance / cleanup.** The `armature-admin` search box submitted `?q=` but the handler read `search` — the UI search was silently inert; the param is aligned and a real `?export=csv` handler added (both now covered by end-to-end tests). Incomplete retractions completed (`armature-analytics`' unused `prometheus` optional dep, `armature-features`' unused `futures` optional dep removed). Adversarially-verified dead code deleted (`RateLimitMiddlewareBuilder`, admin `ListParams::per_page/offset`, collab `record_operation`, analytics' now-superseded public `record_request`/`record_error` methods, i18n `CurrencyFormatter::symbol_before` + `PluralRules::categories` — the last two folding into i18n's already-breaking `0.2.0`). `armature-toon`'s HTTP response paths (`Toon::into_response`, `HttpResponse::toon`, `toon_with_status`) misclassified serialize failures as `DeserializeError` via the deserialize-biased `From`; they now route through `ToonError::from_serialize` for correct classification. Several regression tests found to be non-discriminating (collab merge determinism/delete-preservation, opentelemetry dimension recording, features bucket uniformity) were rewritten to genuinely fail against the pre-fix code.

**Known follow-ups (not blocking):** normal CI does not build `armature-ratelimit`'s `redis` feature or `armature-features`' `launchdarkly` feature, so those crates' container/SDK-gated tests aren't enforced on the default pipeline (pre-existing CI-matrix gap); and `armature-audit`'s `MemoryBackend` remains unbounded (documented test-only).
...
#### Web & API conformance, fourth pass (Workflow 7, project `/audit` + third full 7-domain `/code-review`, then a grpc-scoped re-review)

A conformance `/audit` (name/doc/signature vs implementation, project-scoped rather than diff-scoped) plus a third full 7-domain `/code-review` verified the WF7 rework as genuine and closed a further set of issues the diff-scoped reviews structurally couldn't reach:

**The gRPC compression subsystem's aggregate cap was itself a regression** — the third pass's "running whole-body total" (added to bound aggregate memory) terminated legitimate long/streaming RPCs mid-stream with `resource_exhausted` once cumulative throughput crossed the cap, even though each message is emitted and freed as it flows so concurrent memory never approaches it. Since the streaming `CompressionBody` already frees each message, the **per-message (peak) bound is the correct and sufficient guarantee**; the aggregate counter is removed. A `GatedBody` test (yields message 1, then `Poll::Pending`, then message 2) now proves messages are emitted incrementally rather than buffered to stream-end. Related compression hardening: the default `max_decompressed_size` is lowered from 64 MiB to 4 MiB to match the `max_recv_message_size` default (previously a compression-enabled server silently allowed 16× the configured receive limit in decompressed memory per message); the pre-buffering declared-length guard and the zstd decoder window ceiling both gained headroom so they act as DoS backstops without wrongly rejecting incompressible frames or frames from third-party zstd encoders whose actual output is within the cap; a `usize::MAX` cap no longer overflows the `Read::take` limit; and the duplicated encoder/decoder window-log arithmetic is unified into one `window_log_for` helper so the security invariant (encoder window ≤ decoder ceiling) is structurally guaranteed.

**Two crate READMEs advertised a Quick Start that did not compile** — `armature-graphql` (`#[Query]`/`GraphQLSchema::build`/`graphql_handler`/`playground_handler`/`Application`, none of which exist) and `armature-http-client` (`HttpClient::new().timeout().retry(3).build()` and a `CircuitBreakerConfig.timeout` field, none of which exist) — both rewritten to the real APIs already shown correctly in each crate's `lib.rs` doctests. This is the same "documented primary usage doesn't compile" class that the first WF7 pass fixed for `armature-openapi`, still latent in two siblings.

**Other conformance fixes:** `armature-http-client`'s `AuthInterceptor::intercept` **panicked on a malformed credential** (`.parse().unwrap()` on a token/header with invalid bytes) instead of returning `Err` — now propagated, matching its safer gRPC sibling. `armature-openapi`'s `swagger_ui_response` **double-corrupted every backslash** in the spec (a vestigial template-literal escape doubling `\` into `\\`) and left a `</script>` in any string value able to break out of the page — now injects the raw (valid-JS) JSON with only `</` neutralized to `<\/`. `armature-mcp`'s `GET /mcp/tools`/`/mcp/resources` and `handle_info` counts **read the compile-time registry only**, silently omitting dynamically-registered providers that the JSON-RPC `tools/list` path included — the GET handlers now serve the same merged view, and per-request schema re-parsing is cached at the registry level. `armature-graphql`'s `resolve_entities` now surfaces a subgraph's `errors[]` messages instead of a generic "Invalid response", and its schema-docs page re-escapes SDL before assigning `innerHTML`. `armature-graphql-client`'s advertised `batch()` gained real conformance tests, and its previously-vestigial `Subscription` type is now a working client-initiated unsubscribe (explicit and on-drop, sending a graphql-ws `Complete`). `armature-websocket`'s `ConnectionState::Closed` is now actually reached on teardown (was unreachable), its rustls-only convention is enforced (the opt-in `native-tls` feature and deps removed), and the `#[mcp]` inventory auto-discovery path gained a real dispatch integration test (previously compile-tested only). Ten public enums across the six crates that lacked `#[non_exhaustive]` (against the workspace convention) are now sealed; the gRPC `Interceptor` trait's Logging/Metrics/RequestId impls — inert in the request path — are documented as manually-invoked helpers (with `RequestIdInterceptor` now re-exported), rather than implying an automatic pipeline.

**Version bumps:** `armature-websocket` → `0.2.1` (behavior changes: `is_closed()` semantics, `max_message_size`/heartbeat/`connection_timeout` enforcement, the `Closed`-state wiring; `armature-collab`'s pin updated to match). All other touched crates already carried a WF7 minor bump covering these changes.
